...
function Particles({ params, textureFile, elementsTexUrls }) {
  ...
  // ------------------- 获取纹理尺寸 -------------------
  /**
   * 当 defaultTexture 加载完成后，获取其尺寸并更新 textureSize
   */
  useEffect(() => {
    if (defaultTexture?.image) {
      const { width, height } = defaultTexture.image;
      // Maintain aspect ratio relative to a reference size (optional)
      const aspectRatio = width / height;
      const referenceHeight = 6; // You can adjust this reference size
   ...
   */
  useEffect(() => {
    if (materialRef.current && defaultTexture && pixelsArray.every((tex) => tex?.isTexture)) {
      materialRef.current.uniforms.uVideoTexture.value = defaultTexture;
      materialRef.current.uniforms.uElementTexture.value = Array.isArray(pixelsArray)
        ? pixelsArray
        : [pixelsArray]; // 确保是数组
  ...
  }, [defaultTexture, pixelsArray, fragmentShader]);
